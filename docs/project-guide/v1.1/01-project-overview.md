...
- **데이터 접근은 ORM 과 Raw SQL 두 갈래를 제공하되, 갈라지는 곳은 Repository 하나뿐이다.**
...
| 7 | catalog | **ORM 예제** — 상품 CRUD (`BaseRepository`) | `/api/v1/catalog` |
| 8 | reports | **Raw SQL 예제** — 일별 매출 집계 (`RawRepositoryBase`) | `/api/v1/reports` |

마지막 두 개는 도메인 기능이 아니라 **학습용 예제**다. 같은 구조에서 데이터 접근만
갈렸을 때 코드가 어떻게 달라지는지를 나란히 보여준다. 자세한 것은
[ORM/Raw 선택 기준](09-orm-vs-raw-decision.md) 을 본다.
...
│   │   └── repositories/         # BaseRepository(ORM) + RawRepositoryBase(Raw)
│   ├── features/                 # 도메인별 기능 앱
...
├── repositories/                 # 영속성 연산 (ORM 또는 Raw)
...
ORM/Raw 이중 Repository 는 **구현됐다**(v1.0 시점에는 계획이었다).

| 갈래 | Base | 정의 파일 |
|---|---|---|
| ORM | `BaseRepository[Model]` | `app/core/repositories/repository_base.py` |
| Raw SQL | `RawRepositoryBase` | `app/core/repositories/raw_repository_base.py` |

두 갈래는 Repository 안쪽만 다르다. Dependency → Service → Repository 흐름, 세션 선택,
트랜잭션 경계, Pydantic 검증, 예외 처리, 테스트 기준은 **동일하다**. 그래서 한쪽을
익히면 다른 쪽으로 넘어갈 때 새로 배울 것이 Repository 하나뿐이다.

설계 배경과 결정 근거(ADR)는 `docs/crp/groups/orm-raw-repository/design-baseline.md` 에
있다. 그 문서는 *"왜 이렇게 정했나"* 를 다루고, 이 가이드는 *"어떻게 쓰나"* 를 다룬다.

구현 여부를 판단할 때는 문서 제목이 아니라 실제 코드와 테스트를 우선한다.
...
- **ORM 과 Raw 중 무엇을 고를까: [ORM/Raw 선택 기준](09-orm-vs-raw-decision.md)**

## 변경 이력

| 문서 버전 | 작성일 | 변경 내용 |
|---|---|---|
| 1.1.0 | 2026-08-20 | 대상 저장소 이름 정정, ORM/Raw 두 갈래 반영, 예제 앱 2종(`catalog`·`reports`) 추가, §6 을 "계획" 에서 "구현됨" 으로 |
