...
| REQ-017 | 2026-09-21 | "남은 작업, 고려해야 하는 사항 정리해줘" → "진행해줘" | `get_writer_db_session()`·`get_read_only_db_session()` 의 죽은 `try/except/rollback` 을 지우고, 예외 경로에서 ROLLBACK 이 정확히 1회인지 회귀 테스트로 고정한다 | Active | ADR-026 |
| REQ-018 | 2026-09-21 | 신규 공개된 `anyio` 권고 3건(CVE-2026-63374 · CVE-2026-64847 · CVE-2026-63349, fix 4.14.2)을 해소 | `anyio` 를 4.14.2 이상으로 올려 이 3건을 없앤다. `anyio` 는 starlette·httpx·watchfiles 를 통해 들어오는 **전이 의존**이므로 `pyproject.toml` 에는 손대지 않고 `uv.lock` 만 갱신한다. | Active | ADR-027 |
| REQ-019 | 2026-09-21 | "pytest도 9로 테스트 및 고도화를 진행해줘" | 테스트 하네스를 `pytest` 9.1.1 · `pytest-asyncio` 1.4.0 으로 올린다. `pytest` 는 PYSEC-2026-1845(fix 9.0.3)를 함께 해소하고, `pytest-asyncio` 는 0.x → 1.x 메이저 전환이라 제거된 `event_loop` fixture 를 재정의하는 곳이 있는지 먼저 실측한다. `pytest-cov` 6.3.0 · `pytest-env` 1.2.0 은 pytest 9 에서 그대로 동작하면 건드리지 않는다. 기준선 659 passed / 22 deselected 를 한 건도 잃지 않는다 | Active | ADR-028 |
| REQ-020 | 2026-09-21 | "해결 방안을 제시하고 다음 작업을 정리해줘" → "진행해줘" | 남은 공급망 권고 10건(starlette 7 · sqladmin 2 · aiomysql 1)을 한 번에 해소한다. `starlette` 0.46.2 → 1.6.0, `sqladmin` 0.24.0 → 0.32.0, `aiomysql` 0.2.0 → 0.3.2. `sqladmin` 을 같이 올리는 것이 필수다 — 0.24.0 은 starlette 제약을 안 적어 1.x 를 막지도 지원하지도 않는다. 기준선 659 passed / 22 deselected · `-m mysql` 15 passed 를 한 건도 잃지 않는다 | Active | ADR-029 |
| REQ-021 | 2026-09-21 | "해결 방안을 제시하고 다음 작업을 정리해줘" → "진행해줘" | 재발 방지 — 의존성 감사를 게이트 단계로 들인다. `pip-audit` 을 dev 의존에 추가하고 `scripts/review_gate.py` 에 [5/7] `pip_audit --strict` 단계를, CI `gate` job 에 같은 단계를 넣는다. 상향(REQ-020) 뒤에 넣는다 — 먼저 넣으면 기존 권고 10건 때문에 처음부터 빨간다 | Active | ADR-030 |
...
| ADR-026 | 2026-09-21 | 로깅이 없는 세션 Dependency 2종(`get_writer_db_session`·`get_read_only_db_session`)에서 `try/except Exception: await session.rollback(); raise` 를 지우고 `yield session` 한 줄로 줄인다. `using_writer()`·`mark_read_only()` 는 유지한다. 로깅이 들어 있는 `get_routed_db_session()`·background 계열은 그대로 둔다 — 그쪽은 except 블록이 로그를 남기는 자리다 | SQLAlchemy 2.0.51 의 `AsyncSession.__aexit__` 은 `asyncio.shield(create_task(self.close()))` 이고, `close()` 는 `engine/base.py` 의 `_close_impl` 에서 `if self.is_active: self._connection_rollback_impl()` 로 ROLLBACK 을 이미 보낸다. 엔진 이벤트 실측에서 명시 rollback 이 있으나 없으나 ROLLBACK 은 정확히 1회였다. 게다가 `except Exception` 은 클라이언트 연결이 끊길 때 오는 `asyncio.CancelledError`(BaseException 상속)를 못 잡는데 `__aexit__` 의 shield 는 잡는다 — 지우는 쪽이 예외 안전성이 더 넓다. ADR-008 의 "롤백은 세션 Dependency 의 teardown 이 담당한다" 는 그대로 성립한다 | Accepted | |
| ADR-027 | 2026-09-21 | 전이 의존의 취약점은 `uv lock --upgrade-package <이름>` 으로 **lock 만** 올린다. `pyproject.toml` 에 하한 제약을 추가해 직접 의존으로 끌어올리지 않는다. 이번 `anyio` 권고 3건도 이 방식으로 4.14.0 → 4.14.2 만 갱신했다 | `anyio` 는 starlette 0.46.2 · httpx 0.28.1 · watchfiles 1.2.0 가 요구하는 전이 의존이다. 여기에 우리 하한을 적어두면 upstream 이 요구 범위를 바꿀 때 **우리 제약과 충돌**하고, 그 충돌은 해소가 급한 다음 보안 상향을 막는다. 실측으로 `--upgrade-package anyio` 만으로 4.14.2 가 들어옴을 확인했고 lock diff 는 `anyio` 한 패키지로 한정됐다(다른 패키지 버전 변화 0). 다만 uv 가 `requires-python >= 3.13` 아래에서 해석을 갈라 `python_full_version < '3.15'` → 4.14.2, `>= '3.15'` → 4.15.1 두 갈래를 적었다 — 양쪽 모두 수정 버전 이상이라 권고는 어느 인터프리터에서도 해소된다 | Accepted | |
| ADR-028 | 2026-09-21 | **직접** 의존인 테스트 하네스는 ADR-027 과 반대로 `pyproject.toml` 의 하한을 올려 고정한다 — `pytest>=9.0.3,<10.0.0` · `pytest-asyncio>=1.0.0,<2.0.0`. 하한 9.0.3 은 PYSEC-2026-1845 의 수정 버전이고, 상한은 다음 메이저를 막는다. `pytest-asyncio` 1.x 가 `event_loop` fixture 를 제거했으므로 `asyncio_mode = "auto"` 는 유지하되 `asyncio_default_fixture_loop_scope = "function"` 을 **명시**한다. `pytest-cov`·`pytest-env` 는 현행 유지 | ADR-027 의 "lock 만" 규칙은 upstream 이 요구 범위를 정하는 **전이** 의존에만 성립한다. 개발 의존은 우리가 유일한 소비자라 `pyproject.toml` 에 적지 않으면 재현 가능한 하한이 없고, 보안 권고의 수정 버전이 어디인지도 남지 않는다. `event_loop` 재정의는 저장소 전체 grep 으로 0건임을 확인했다(`asyncio.new_event_loop()` 2곳은 `app/celery/task.py` 의 워커 루프와 `tests/core/test_runtime_lifecycle.py` 의 순수 asyncio 코드로, pytest-asyncio fixture 가 아니다) — 그래서 코드 변경 없이 제약과 설정만으로 전환된다. `asyncio_default_fixture_loop_scope` 는 1.4.0 에서 미설정 시 경고를 내지 않지만 암묵적 기본값이 향후 바뀔 예정이라, 지금 실제로 도는 값("function")을 못 박아 조용한 동작 변화를 막는다. 실측: 659 passed / 22 deselected 유지, `-m mysql` 15 passed, 신규 경고 0, `uv.lock` 변화는 `pytest`·`pytest-asyncio` 두 패키지로 한정 | Accepted | |
| ADR-029 | 2026-09-21 | 전이 의존(`starlette`)의 메이저 상향은 **그것을 업고 있는 직접 의존이 새 대역을 명시적으로 지원할 때까지 묶어서** 올린다. 이번엔 `starlette` 1.6.0 · `sqladmin` 0.32.0 을 한 록에 넣었다. `aiomysql` 은 직접 의존이므로 ADR-028 과 같이 `pyproject.toml` 하한을 `>=0.3.2,<0.4.0` 로 올린다 | `sqladmin` 0.24.0 은 `starlette` 를 버전 제약 없이 요구한다 — 1.x 를 설치해도 resolver 가 막지 않지만 지원한다고 선언하지도 않아 암묵적 미지원 조합이 된다. 0.31.0·0.32.0 은 `starlette>=0.50,<2.0` 을 명시해 1.x 를 공식 지원한다. `fastapi` 0.141.1 은 `starlette>=0.46.0` 에 **상한이 없어** 1.6.0 을 받는다. `aiomysql` 0.2.0 → 0.3.2 는 드라이버 교체라 SQLite 단위테스트로는 증명되지 않는다 — 실제 MySQL 8.4 로 `-m mysql` 15건을 돌려 확인했다. 실측: lock 변화는 세 패키지로 한정, 신규 전이 의존 0개, 659 passed / 22 deselected · `-m mysql` 15 passed · 브라우저 7 ERROR(Redis 미가용 — 상향 전후 동일) | Accepted | |
| ADR-030 | 2026-09-21 | 의존성 감사는 `--fast` 에도 남긴다 — `[5/7] pip-audit`. `--strict` 를 쓴다(감사하지 못한 패키지가 있으면 실패). 위치는 bandit 바로 뒤, pytest 앞이다. CI `gate` job 에도 같은 명령을 넣는다 | 이 게이트의 `--fast` 기준은 "느린 것"이 아니라 **"외부 인프라(docker MySQL · Chromium+Redis)가 있어야 도는 것"** 이다. pip-audit 은 인프라가 필요 없고 실측 2.4초라 그 기준에 걸리지 않는다. 더 중요한 것은 이 단계가 잡는 게 **코드 변경과 무관하게 새로 뜨는 권고**라는 점이다 — 전체 게이트에만 두면 docker 를 올리는 사람만 보게 되고, 그것이 REQ-020 에서 권고 10건이 쌓일 때까지 보이지 않았던 이유다. `--strict` 는 ADR-023 의 "skip 을 실패로 본다" 와 같은 기준이다 — "검사 못 했다"가 초록으로 보이면 게이트가 거짓말이 된다. 단계가 실제로 빨개지는지는 `starlette==0.46.2` 를 담은 requirements 로 돌려 exit 1 을 확인했다. `pip-audit` 자체가 dev 전이 의존 20개(requests · urllib3 · cyclonedx 등)를 데려오지만 그것들도 감사 대상에 들어오며 현재 0건이다 | Accepted | |
...
- v1.3 (2026-09-21): REQ-017 · ADR-026 등재 — 세션 Dependency 2종의 죽은 `try/except/rollback` 삭제, 예외 경로 ROLLBACK 1회 회귀 테스트 추가, ARCHITECTURE §4.1·feature-development-guide.html 의 롤백 주체 서술 정정.
- v1.4 (2026-09-21): REQ-018 · ADR-027 등재 — 신규 `anyio` 권고 3건을 `uv lock --upgrade-package anyio` 로만 해소(4.14.0 → 4.14.2, `pyproject.toml` 무변경). 전이 의존 취약점의 처리 방식을 ADR 로 고정.
- v1.5 (2026-09-21): REQ-019 · ADR-028 등재 — 테스트 하네스를 `pytest` 8.4.2 → 9.1.1(PYSEC-2026-1845 해소) · `pytest-asyncio` 0.24.0 → 1.4.0 으로 상향. `event_loop` 재정의 0건이라 테스트 코드는 무변경이고, `asyncio_default_fixture_loop_scope = "function"` 만 명시했다. 직접 의존은 `pyproject.toml` 하한을 올린다는 점에서 ADR-027 과 짝을 이룬다.
- v1.6 (2026-09-21): REQ-020 · ADR-029 등재 — 공급망 권고 10건 해소. `starlette` 0.46.2 → 1.6.0 · `sqladmin` 0.24.0 → 0.32.0 · `aiomysql` 0.2.0 → 0.3.2 를 **한 록**으로 올렸다(sqladmin 이 starlette 1.x 를 명시 지원하는 버전으로 같이 가야 하기 때문). 코드 변경 0건, `StarletteDeprecationWarning` 2건은 경고로만 남겨 다음 작업으로 넘긴다.
- v1.7 (2026-09-21): REQ-021 · ADR-030 등재 — 게이트에 의존성 감사 단계 추가. `pip-audit>=2.7` 을 dev 의존에 넣고 `review_gate` 를 8단계 → 9단계(`--fast` 6 → 7)로, CI `gate` job 을 같은 모양으로 확장했다. 상향(v1.6) 뒤에 넣어 첫 실행이 0건이다.
- v1.8 (2026-09-21): ADR-029 후속 — `ValidationException.status_code` 를 `HTTP_422_UNPROCESSABLE_ENTITY` 에서 `HTTP_422_UNPROCESSABLE_CONTENT` 로 옮겼다. starlette 1.x 가 옛 이름을 폐기 예고했고(값은 같은 422), 이 상향이 드러낸 마지막 자체 코드 경고였다. 남은 경고 1건은 `fastapi.testclient` 의 httpx 폐기 예고로 서드파티다.
