...
CRUD 앱의 공통 흐름: Create(스키마 검증 → create/flush → 핸들러 commit → 201), List(pagination → count + 목록, commit 없음),
Retrieve(없으면 기능별 Not Found), Update(변경 필드만 → flush → commit), Delete(삭제 → commit → 204).
