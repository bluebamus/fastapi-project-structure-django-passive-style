...
| REQ-S09 | 2026-09-18 | "debug 모드에서는 sql과 바인딩 값 등 추적에 필요한 모든 정보를 출력해줘. 이 외에는 제안한 보안문제가 개선되도록 수정해줘" | 세션 롤백 경로는 DEBUG 레벨에서 SQL·바인딩 값·트레이스백 전문을 남기고, ERROR 레벨에는 예외 타입만 남긴다. 그 대가로 `ENV` 가 staging/production 이면 `DEBUG=true` 또는 `LOG_LEVEL=DEBUG` 를 기동 거부한다. 저장소 전체에서 예외 메시지를 f-string 으로 찍는 로그를 같은 기준(`%s` 지연 포매팅 + 타입만)으로 정리한다 | Active | ADR-S10 |
| REQ-S10 | 2026-09-21 | "남은 작업, 고려해야 하는 사항 정리해줘" → "진행해줘" | 배포 안전 검사가 `MYSQL_PASSWORD`·`REDIS_PASSWORD`·`SMTP_PASSWORD` 까지 본다. MySQL 은 빈 값도 위반, Redis·SMTP 는 값이 있을 때만 예시값인지 본다. `.env.example` 의 MySQL 비밀번호 예시를 검증기가 실제로 잡는 placeholder 로 바꾼다 | Active | ADR-S11 |
...
| ADR-S10 | 2026-09-18 | 추적 정보는 **새 설정 없이** 로그 레벨로 가른다 — `get_routed_db_session()`·`get_background_session()` 의 기존 `logger.error` 바로 뒤에 `logger.debug("... ROLLBACK 상세", exc_info=True)` 한 줄을 둔다. ERROR 레코드의 지연 포매팅·타입만 남기는 판단(C-5)은 그대로 둔다. `get_background_session()` 은 `f"error: {type(e).__name__}: {e}"` 로 예외 메시지를 그대로 찍고 있었다 — 여러 줄 f-string 이라 `logger.` 가 같은 줄에 없어서 `test_log_messages_do_not_interpolate_exception_values` 의 `logger.` 조건에 걸리지 않았다. 그 조건을 없애 검사 범위를 파일 전체로 넓혔다. 거부 조건 2건은 **기존** `validate_deployment_safety()` 의 `problems` 수집에 붙이고 예외 종류(`ValueError`)·"설정 이름만" 규약을 유지한다 — 메시지 머리말만 `비밀키 설정이` → `배포 설정이` 로 넓혔다. 테스트는 기존 파일 2곳에 합쳤다(`test_deployment_safety.py` 2건, `test_security_hardening.py` caplog 1건). pytest 는 `DEBUG=true` 로 돌므로(pyproject `env`) 기존 배포 안전 fixture 와 자식 프로세스에 `DEBUG=false` 를 명시했다 | debug 모드에서 추적 정보를 보여 달라는 요청은 곧 "운영에서 SQL 이 로그에 남는 스위치"를 만드는 일이다. 스위치를 새로 만들지 않고 기존 로그 레벨에 얹으면 소비처가 늘지 않고, 대신 그 레벨이 배포에서 켜지지 못하게 같은 자리(import 시점 모듈 함수)에서 막으면 대가가 상쇄된다. `DEBUG` 는 `AppSettings`, `LOG_LEVEL` 은 `LogSettings` 라 한 클래스의 validator 로는 볼 수 없다 — ADR-S09 와 같은 이유로 같은 함수에 둔다 | Active | ADR-S09 |
| ADR-S11 | 2026-09-21 | 기존 `validate_deployment_safety()` 의 `secrets` 수집에 `MYSQL_PASSWORD` 를 그대로 넣고(빈 값 = 위반), `REDIS_PASSWORD`·`SMTP_PASSWORD` 는 `strip()` 후 값이 있을 때만 넣는다. 새 함수·새 설정은 만들지 않고 기존 `is_placeholder_secret()` 판정과 "설정 이름만" 규약을 그대로 쓴다. `.env.example` 의 `MYSQL_PASSWORD=your_password` 는 `change-this-mysql-password` 로 바꾼다 | `your_password` 는 `your-` 로 시작하지 않아(`_`) 판정을 통과했다 — `.env.example` 을 그대로 들고 가도 MySQL 비밀번호만 조용히 지나갔다. 빈 값의 의미는 설정마다 다르다: 운영 MySQL 에 비밀번호 없이 붙는 것은 그 자체가 사고지만, 인증 없는 사설망 Redis 와 발송 모듈이 없는 SMTP 는 정당한 배포다(`REDIS_URL` 이 이미 비밀번호 유무로 분기한다). 빈 값을 일괄 위반으로 보면 멀쩡한 배포가 기동하지 못한다. 테스트는 기존 `tests/core/test_deployment_safety.py` 에 합치고, 로컬 `.env` 에 예시값이 남아 있어도 흔들리지 않도록 `STRONG` 에 비밀번호 3종을 명시했다 | Accepted | ADR-S09 |
...
| 2026-09-18 | REQ-S09 · ADR-S10 — 세션 롤백 DEBUG 상세 레코드(`exc_info=True`) 추가, `get_background_session()` 의 예외 메시지 f-string 제거, staging/production 에서 `DEBUG=true`·`LOG_LEVEL=DEBUG` 기동 거부(`validate_deployment_safety()`), 예외 메시지 보간 검사 범위 확대, README·ARCHITECTURE §3.2/§5/§11.1·server-lifecycle-guide.html 반영 |
| 2026-09-21 | REQ-S10 · ADR-S11 — 배포 안전 검사에 `MYSQL_PASSWORD`(빈 값 포함)·`REDIS_PASSWORD`·`SMTP_PASSWORD`(값이 있을 때만) 추가, `.env.example` MySQL 비밀번호 예시를 검증기가 잡는 placeholder 로 교체 |
